...
var _number = _interopRequireDefault(require("../../util/number"));
...
function validate(type, min, max, step, partial = false) {
  if (type === FIXED_CODE) {
    ...
    if (type === STEPPED_CODE) {
      if (!_number.default.isMultiple(step, min)) errors.push(`${type} batch minimum must be a multiple of the batch step size.`);
      if (!_number.default.isMultiple(step, max)) errors.push(`${type} batch maximum must be a multiple of the batch step size.`);
    }
