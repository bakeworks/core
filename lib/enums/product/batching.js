 const isMultiple = require('../../util/number').isMultiple
...
const ALL = [NONE, FIXED, VARIABLE, STEPPED]
...
function none() {
  const inst = instantiate(NONE, 0, 0, 0)
  return inst
}
...
function validate(type, min, max, step, partial = false) {
  if (type === FIXED) {
    ...
    if (type === STEPPED) {
      if (!isMultiple(step, min)) errors.push(`${type} batch minimum must be a multiple of the batch step size.`)
      if (!isMultiple(step, max)) errors.push(`${type} batch maximum must be a multiple of the batch step size.`)
    }
...
module.exports = {
  NONE, FIXED, VARIABLE, STEPPED, ALL,
  DEFAULT_MAX_SIZE,
