...
function validate(type, min, max, step) {
  ...
  if (type === FIXED_CODE) {
    if (min <= 0) return "".concat(type, " batch size must be greater than zero.");
  } else if (type === VARIABLE_CODE || type === STEPPED_CODE) {
    var errors = [];
    if (min >= max) errors.push("".concat(type, " batch minimum must be less than the maximum."));

    if (type === STEPPED_CODE) {
      if (!number.isMultiple(step, min)) errors.push("".concat(type, " batch minimum must be a multiple of the batch step size."));
      if (!number.isMultiple(step, max)) errors.push("".concat(type, " batch maximum must be a multiple of the batch step size."));
    }

    if (partial) errors.push("Only ".concat(FIXED_CODE, " size batches may be partial."));
...
function normalise(type, min, max, step, partial) {
  ...
  switch (type) {
    ...
    default:
      throw new Error("unhandled batch size type ".concat(type, "'"));
  }
}
