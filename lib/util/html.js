function tag(t, content) {
  ...
  var styleStr = style ? " style=\"".concat(style, "\"") : '';
  return "<".concat(t).concat(styleStr, ">").concat(typeof content === 'function' ? content() : content, "</").concat(t, ">");
}
...
function objectsToHtmlTable(objs) {
  ...
  var keys = args.keys === undefined ? Object.keys(objs[0]) : args.keys; // console.log(`Object.keys(objs[0]) = ${Object.keys(objs[0])} || keys = ${keys}`)
