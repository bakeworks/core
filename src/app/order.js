import $csv from 'bakeworks-core/src/util/csv'
import $date from 'bakeworks-core/src/util/date'
...
function xItemTotal (qtys) {
  return DAY_TAGS.reduce((sum, day) => sum + parseInt(qtys[day], 10), 0)
}

function xItemSpecialDayCount (item) {
  let sum = 0
  DAY_TAGS.forEach((day) => {
    if (isItemSpecialQuantity(item, day) && parseInt(item.current[day], 10) > 0) {
      sum += 1
    }
  })
  return sum
...
function xSetItemQuantity (qtys, day, qty) {
  // eslint-disable-next-line no-param-reassign
  qtys[day] = parseInt(qty, 10)
  // eslint-disable-next-line no-param-reassign
  qtys.tot = xItemTotal(qtys)
}

function setItemQuantity (item, day, qty) {
  xSetItemQuantity(item.current, day, qty)
  // keep standing and current identical if order is standing
  if (isItemStanding(item)) {
    xSetItemQuantity(item.standing, day, qty)
  }
}
...
function specialItemsCount (order) {
  return order.items.reduce((count, item) => count + xItemSpecialDayCount(item), 0)
}
...
function nearbyPeriods (dateFormat = PERIOD_FORMAT) {
  const standing = {
    isStanding: true,
    index: null,
    start: null,
...
function matchingSpecial (standingOrder, specialOrders) {
  return specialOrders.find(e => e.customer_id === standingOrder.customer_id
...
function mapQtysToDays (q) {
  ...
  result[WEEK_TOT_TAG] = q.reduce((sum, v) => sum + parseInt(v, 10), 0)
  return result
}
